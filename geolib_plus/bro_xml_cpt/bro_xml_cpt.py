...
class BroXmlCpt(AbstractCPT):
    ...
    def __parse_bro(self, cpt, minimum_length=5, minimum_samples=50, minimum_ratio=0.1, convert_to_kPa=True):
        """
    ...
    def __define_pre_drill(self, cpt_BRO, length_of_average_points=3):
        ...
        if depth[0] > 0:
            ...
            coneresistance = np.append(np.average(cpt_BRO['dataframe']['coneResistance'][:length_of_average_points]),
                                       coneresistance)
            frictionratio = np.append(np.average(cpt_BRO['dataframe']['frictionRatio'][:length_of_average_points]),
                                      frictionratio)
            localfriction = np.append(np.average(cpt_BRO['dataframe']['localFriction'][:length_of_average_points]),
                                      localfriction)
            ...
            for water_measurement_type in self.__water_measurement_types:
                if water_measurement_type in cpt_BRO["dataframe"]:
                    pore_pressure = np.append(
                        np.average(cpt_BRO['dataframe'][water_measurement_type][:length_of_average_points]),
                        pore_pressure)
