    ...
    def information(self, qcdq, lithology):
        ...
        for lit in lithology:
            if int(lit) == 1:
                ...
                new_lit.append([["Zand", "Klei"],
                                [["Sterk siltig, kleiig"], ["Sterk zandig"]]])
            elif int(lit) == 5:
                new_lit.append([["Zand"],
                                [["Zwak siltig, kleiig"]]])
            elif int(lit) == 6:
        ...
        for idx in range(len(qcdq)):
            ...
            # append to result
            result["litho_NEN"].append("/".join([qcdq_table[i][0], qcdq_table[i][1], qcdq_table[i][2]]))
            result["E_NEN"].append("/".join(list(set(self.soil_types_list[qcdq_table[i][0]][qcdq_table[i][1]][qcdq_table[i][2]]["E"]))))
            result["cohesion_NEN"].append("/".join(list(set(self.soil_types_list[qcdq_table[i][0]][qcdq_table[i][1]][qcdq_table[i][2]]["Cohesie"]))))
            result["fr_angle_NEN"].append("/".join(list(set(self.soil_types_list[qcdq_table[i][0]][qcdq_table[i][1]][qcdq_table[i][2]]["friction_angle"]))))
